...
from backend.app.transaction.utils import TransactionFailureReason
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": (
                        ...
                        " fraudulent. An account executive will review the transaction, "
                        "before it is either approved or rejected."
                    ),
                    "risk_analysis": {
    ...
    try:
        ...
        if not all([sender_account, receiver_account, sender, receiver]):
            await mark_transaction_failed(
                ...
                session=session,
                error_message="Account informatin not found",
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        await session.rollback()
        logger.error(f"Failed to complete tranfer: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_user_statement_data(
    user_id: uuid.UUID, start_date: datetime, end_date: datetime, session: AsyncSession
    ...
    try:
        ...
        for txn in transactions:
            sender_account = await session.get(
                BankAccount, txn.sender_account_id if txn.sender_account_id else None
            )
            receiver_account = await session.get(
                BankAccount,
                txn.receiver_account_id if txn.receiver_account_id else None,
            )
...
async def _complete_approved_transfer(transaction: Transaction, session: AsyncSession):
    try:
        ...
        try:
            ...
            await session.rollback()
            raise ValueError(f"Failed to process transfer: {e}")
        ...
        await session.rollback()
        logger.error(f"Unexpected error in _complete_approved_transfer: {str(e)}")
        raise
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to send transfer withdrawal approval notification: {e}"
                )

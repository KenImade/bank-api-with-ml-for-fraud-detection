...
async def create_virtual_card(
    ...
    try:
        ...
        cleaned_data.pop("total_popped_up", None)
        cleaned_data.pop("card_metadata", None)

        card_number = generate_visa_card_number()

        if not cleaned_data.get("expiry_date"):
            expiry_data = generate_card_expiry_date()
            cleaned_data["expiry_date"] = expiry_data.date()
...
async def activate_virtual_card(
    ...
    try:
        ...
        if not executive or executive.role != RoleChoicesSchema.ACCOUNT_EXCECUTIVE:
            raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"status": "error", "message": "Failed to activate virutal card"},
        )
...
async def delete_virtual_card(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"status": "error", "message": "Failed to delete virutal card"},
        )

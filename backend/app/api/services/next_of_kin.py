...
from backend.app.next_of_kin.schema import (
    NextOfKinCreateSchema, NextOfKinReadSchema, NextOfKinUpdateSchema
)
...
async def create_next_of_kin(
    ...
    try:
        ...
        logger.info(f"Next of kin create successfully for user: {user_id}")
...
async def get_user_next_of_kins(
    ...
    try:
        ...
        logger.error(f"Failed to retrieve next of kin: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "status": "error",
                "message": "Failed to get user next of kins"
            }
        )
...
async def delete_next_of_kin(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                "status": "error",
                "message": "Failed to delete next of kin details",
            }
        )
